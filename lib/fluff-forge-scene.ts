...
import { drawChinchilla, type ChinOpts as ChinArt } from './chinchilla-art';
...
export type ChinOpts = ChinArt & { power?: Power };

/** Dora or Enzo from the shared drawing in chinchilla-art.ts, wearing a power-up if they have one. */
export function chinchilla(c: CanvasRenderingContext2D, id: HeroId, x: number, base: number, o: ChinOpts) {
  const hat = o.power === 'clover' ? (d: CanvasRenderingContext2D, bob: number) => { d.save(); d.translate(1.5, -29 + bob); d.scale(0.5, 0.5); clover(d, 0, 0); d.restore(); }
    : o.power === 'feather' ? (d: CanvasRenderingContext2D, bob: number) => { d.save(); d.translate(0.5, -28 + bob); d.scale(0.6, 0.6); feather(d, 0, 0); d.restore(); }
    : undefined;
  drawChinchilla(c, id, x, base, { ...o, decorate: hat });
}
