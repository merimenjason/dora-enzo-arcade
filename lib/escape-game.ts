...
export class EscapeGame{
 ...
 get solids(){return this.phase==='cage'?[...PLATFORMS.map(p=>({...p,bottom:p.y-.13,top:p.y})),...CAGE_PROPS]:BLOCKS.map(p=>({...p,bottom:0,top:2}))}
 ...
 sees(g:Guard){const dx=this.x-g.x,dz=this.z-g.z,d=Math.hypot(dx,dz);if(d>.8&&this.hidden)return false;if(d>VISION_RANGE||occluded(g,this))return false;const dot=(dx*Math.sin(g.angle)+dz*Math.cos(g.angle))/(d||1);return d<.8||dot>Math.cos(VISION_HALF_ANGLE)}
 retry(){if(this.state!=='caught')return;this.state='playing';this.x=-9;this.z=-5;this.y=0;this.vy=0;this.alert=0;this.badge=false;this.searching=false;this.searchProgress=0;this.decoys=MAX_DECOYS;this.decoy=null;this.decoyTime=0;this.guards=ROUTES.map(r=>({x:r[0].x,z:r[0].z,angle:0,target:1,distracted:0}));this.say('Back by the cage. Both friends are safe. Try another route.')}
 step(dt:number,input={x:0,z:0,sneak:false}){if(!this.active)return;dt=Math.min(dt,1/60);this.time+=dt;this.messageTime=Math.max(0,this.messageTime-dt);this.sneaking=input.sneak;this.moving=Math.hypot(input.x,input.z)>.01;const n=Math.max(1,Math.hypot(input.x,input.z));if(this.moving)this.face={x:input.x/n,z:input.z/n};const speed=this.sneaking?1.65:3.4;const limit=this.phase==='cage'?{x:4.2,z:3.15}:{x:11.05,z:7.85};
