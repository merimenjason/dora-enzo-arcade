...
export const TARGETS={stick:{x:-3,z:0},key:{x:2.8,z:-1},latch:{x:0,z:3},badge:{x:-1,z:-6},exit:{x:10.5,z:-7}};
...
export class EscapeGame{
 state:'ready'|'playing'|'paused'|'caught'|'won'='ready';phase:'cage'|'room'='cage';x=0;z=0;y=0;vy=0;time=0;stick=false;key=false;badge=false;sneaking=false;moving=false;alert=0;noticed=false;decoys=3;decoy:Point|null=null;decoyTime=0;face={x:0,z:1};message='';messageTime=0;guards:Guard[]=ROUTES.map(r=>({x:r[0].x,z:r[0].z,angle:0,target:1,distracted:0}));
 get active(){return this.state==='playing'}
 get hidden(){return this.phase==='room'&&this.sneaking&&HIDES.some(r=>within(this,r,-.35))}
 get objective(){return this.phase==='cage'?!this.stick?'Search the blue tunnel for a chew stick.':!this.key?'Use the stick to open the pink hideout.':'Bring the toy key to the wire door latch.':!this.badge?'Sneak to the desk and collect the exit card.':'Reach the green exit with both chinchillas.'}
 get target(){return this.phase==='cage'?!this.stick?TARGETS.stick:!this.key?TARGETS.key:TARGETS.latch:!this.badge?TARGETS.badge:TARGETS.exit}
 get nearTarget(){return Math.hypot(this.x-this.target.x,this.z-this.target.z)<1.45}
 ...
 interact(){if(!this.active||!this.nearTarget)return false;if(this.phase==='cage'){if(!this.stick){this.stick=true;this.say('Enzo braces the tunnel. Dora pulls out a chew stick.')}else if(!this.key){this.key=true;this.say('The stick lifts the hideout catch. A toy key is inside!')}else{this.phase='room';this.x=-9;this.z=-5;this.y=0;this.vy=0;this.alert=0;this.say('The cage opens. Stay low and find the exit card.')}}else if(!this.badge){this.badge=true;this.say('Exit card found. Reach the green door together.')}else{this.state='won';this.say('Enzo and Dora escaped together!')}return true}
 jump(){if(this.active&&this.y===0){this.vy=4.5;return true}return false}
 ...
 step(dt:number,input={x:0,z:0,sneak:false}){if(!this.active)return;dt=Math.min(dt,1/60);this.time+=dt;this.messageTime=Math.max(0,this.messageTime-dt);this.sneaking=input.sneak;this.moving=Math.hypot(input.x,input.z)>.01;const n=Math.max(1,Math.hypot(input.x,input.z));if(this.moving)this.face={x:input.x/n,z:input.z/n};const speed=this.sneaking?1.65:3.4;const limit=this.phase==='cage'?{x:4.3,z:3.3}:{x:11.2,z:8};const nx=Math.max(-limit.x,Math.min(limit.x,this.x+input.x/n*speed*dt)),nz=Math.max(-limit.z,Math.min(limit.z,this.z+input.z/n*speed*dt));if(this.phase==='cage'||!BLOCKS.some(r=>within({x:nx,z:this.z},r,.55)))this.x=nx;if(this.phase==='cage'||!BLOCKS.some(r=>within({x:this.x,z:nz},r,.55)))this.z=nz;this.vy-=11*dt;this.y=Math.max(0,this.y+this.vy*dt);if(this.y===0)this.vy=0;if(this.phase==='cage')return;
