...
export type AreaId = 'approach' | 'hall' | 'cellar' | 'belfry';
 ...
 * `G` a boss gate (shut during the fight), `D` a sealed door. Things placed on the map: `S` dust-bath shrine
 * (saves and heals), `i` candle, `n` sign, `H` Wolfberry Leaf (max HP up), `R` relic, `I` item, `O` boss,
 * and enemies `b` bat, `m` dust moth, `k` shell beetle, `x` bone mouse, `a` armadillo guard.
 */
export type Room = {
  id: string; area: AreaId; mx: number; my: number; w: number; h: number; rows: string[];
  notes?: string[]; items?: string[]; relic?: RelicId; boss?: BossId;
};
export type RelicId = 'dash';
export type BossId = 'owl';
...
export const ROOMS: Room[] = [
  ...
  }, { notes: ['A great door crusted with old dust. Beyond it lie the Pantry Catacombs, and somewhere far above, the Golden Wolfberry. This chapter of Fluffstevania is still being dug out. Check back soon!'] }),
];
...
export type WeaponStyle = 'whip' | 'claws' | 'club';
...
export const GEAR: Record<string, Gear> = {
  ribbon: { name: 'Ribbon Whip', slot: 'weapon', hero: 'dora', style: 'whip', atk: 2, text: 'A silk ribbon Dora snaps like a whip. Long reach.' },
  bramble: { name: 'Bramble Whip', slot: 'weapon', hero: 'dora', style: 'whip', atk: 7, reach: 8, text: 'A thorny vine, longer and meaner than the ribbon.' },
  claws: { name: 'Scrappy Claws', slot: 'weapon', hero: 'enzo', style: 'claws', atk: 2, text: 'Enzo’s own two paws. Short and very quick.' },
...
export const itemName = (id: string) => GEAR[id]?.name ?? FOOD[id]?.name ?? id;

// ─── Story ────────────────────────────────────────────────────────────────

export type Speaker = HeroId | 'owl' | 'sign';
...
export const SCRIPTS: Record<string, Line[]> = {
  ...
  owlDown: [
    { who: 'owl', text: 'Hoo... hooo... Go on then. The Count waits beyond the catacombs, and he is always, always hungry.' },
...
export const ROOM_SCRIPTS: Record<string, string> = { path: 'intro', hall: 'hall', cellar: 'cellar' };
