...
`chinchilla()` draws Dora or Enzo at any height, which is how the small and big sizes, the HUD portrait and the menu art share one drawing. It is drawn from the photos in `public/art/` (`dora.jpeg`, `enzo.jpeg`, `the-original-duo.jpg`) in a 24-unit frame, side-on: a round loaf of a body with the head set straight into it, and body, haunch, chest, head, muzzle and cheek fluff outlined as one silhouette (stroke every shape thick, then fill them all). `COATS` holds each coat. Dora is white with peach-pink ears you can almost see through, ruby eyes, pink toes and a smooth tail that sweeps out and curls up. Enzo is standard grey with a darker wavy back, a paler face, a white chest and front legs, black eyes and a bottlebrush tail that swells toward the end. `tail()` lays the tail out as circles along a curve, fringed with loose fur. The pose follows the hero: a breathing idle with a twitching nose, a bounding stride with the tail streaming back, ears swept back and paws reaching out in the air, and crossed-out eyes when a try ends.
