...
`drawGame()` draws the backdrop (parallax mountains, clouds, snow or cave sparkles per theme), the visible tiles, lifts, flags, goal, items, enemies, the hero and effects, then the HUD. `drawEditor()` draws the raw course, with enemies, markers and drifting clouds shown where they were placed, a grid, a red line at the course's right edge and a ghost of the selected part under the cursor. Prize blocks all look like `?` blocks in play, and in the editor clover and feather blocks carry a small badge. `chinchilla()` draws Dora or Enzo at any height, which is how the small and big sizes and the menu art share one drawing. It works in a 24-unit frame and outlines the body, haunch, head and cheek fluff as one silhouette (stroke every shape thick, then fill them all). `COATS` holds each coat: Dora is white with a cream-tipped tail, Enzo standard grey with a darker back and white belly. The pose follows the hero: a breathing idle with a twitching nose, a stride and bouncing tail while moving, ears swept back and paws reaching out in the air, and crossed-out eyes when a try ends. Everything is in course pixels, and the page scales the canvas by 2.
