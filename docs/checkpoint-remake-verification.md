...
| R06 | Add arcade menu integration while retaining original | W asserts nine `.arcade-card` elements, the updated “Nine ways to play.” heading, exactly one `/checkpoint-remake` card and one `/checkpoint` card. Both editions are reachable. |
