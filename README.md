...
Pick any of six maverick stages: Snowcap Ridge (Frost Fox), Cloud Forest (Storm Owl), Ember Caldera (Magma Snake), Crystal Mines (Quartz Armadillo), Salt Flats (Volt Vicuña) or Titicaca Falls (Tide Toad). Each has checkpoints, beetles, bats, turrets or hopping frogs, pits and spikes (6 damage each; a pit puts you back on the last safe ground) and a boss room that seals behind you. Beating a maverick gives its weapon: Frost Shard pierces, Gale Feather fires a fan of three, Ember Coil rolls along the ground, Quartz Orbit circles you with three crystals for 3 s that block enemy shots, Volt Spark curves toward the nearest enemy, and Bubble Burst fires two rising bubbles that pass through enemies (28 energy, 2 per shot). Each maverick takes triple damage, and flinches, from another maverick's weapon; the Quartz Armadillo deflects every other shot while it rolls. Each maverick stage hides a heart tank that adds 2 to both heroes' maximum health (up to 28). Clearing all six opens the Cougar Citadel and the Cougar Kingpin. Enemies drop health (+6) and weapon energy (+8). Cleared stages, heart tanks and best times are saved in this browser.
