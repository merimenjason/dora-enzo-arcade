...
Hidden in the stages: a heart tank in each maverick stage (+2 maximum health for both heroes, up to 28); sub-tanks in Cloud Forest, Ember Caldera, Crystal Mines and Titicaca Falls, which store health picked up beyond your maximum (up to 16 each) and heal you from the pause screen; the Dash Boots in the Salt Flats (one air dash per jump); and the Saber Crest in the Crystal Mines (Enzo's slash while dashing becomes a 5-damage dash slash with longer reach). Enemies drop health (+6) and weapon energy (+8).
...
Options on the stage select: easy mode (half damage, and pits don't hurt), two players, music, and a best-time ghost. In two-player co-op Dora and Enzo are both on screen with their own health and weapons and can't tag; a fallen player returns with half health at the next checkpoint, and the run ends when both are down. A single-player run that sets a new best time without a retry is saved as that stage's ghost, which replays translucently alongside you next time. Cleared stages, collectibles, found weaknesses, best times, ghosts, options and key bindings are saved in this browser.
