import assert from 'node:assert/strict';
import { chromium } from 'playwright';
import { mkdir } from 'node:fs/promises';
const browser = await chromium.launch();
const base = process.env.E2E_BASE_URL || 'http://localhost:3000';
const errors = [];
try {
  const page = await browser.newPage({
    viewport: { width: 1440, height: 1100 },
    reducedMotion: 'reduce',
  });
  page.on('pageerror', (e) => errors.push(e.message));
  await page.clock.install();
  await page.goto(`${base}/dust-bath`);
  await page.getByRole('button', { name: /Open spa/ }).waitFor();
  assert.equal(await page.locator('.db-welcome li').count(), 3);
  assert.equal(
    await page
      .locator('.db-welcome > div > p')
      .nth(1)
      .evaluate((el) => getComputedStyle(el).color),
    'rgb(91, 107, 93)',
  );
  assert.equal(
    await page.getByRole('link', { name: /MAIN ARCADE/ }).getAttribute('href'),
    '/',
  );
  await page.getByRole('button', { name: /Untimed cozy/ }).click();
  assert.match(await page.getByTestId('clock').textContent(), /No rush/);
  const seat = async (i = 0) => {
    if (!(await page.locator('.db-guest').count()))
      await page.clock.runFor(8100);
    await page.locator('.db-guest').first().click();
    assert.equal(
      await page.locator('.db-guest').first().getAttribute('aria-pressed'),
      'true',
    );
    await page.locator('.db-bath').nth(i).click();
  };
  const keyboardScrub = async (ms = 1300) => {
    await page.locator('.db-scrub').focus();
    await page.keyboard.down('Space');
    await page.clock.runFor(ms);
    await page.keyboard.up('Space');
  };
  await seat(0);
  await seat(1);
  await page.locator('.db-bath').first().click();
  await keyboardScrub(1900);
  assert.match(await page.getByRole('status').textContent(), /achoo/);
  assert.match(
    await page.locator('.db-bath').nth(1).textContent(),
    /1 splash mess/,
  );
  await page.getByRole('button', { name: /Give selected guest/ }).click();
  assert(
    !/splash mess/.test(await page.locator('.db-bath').first().textContent()),
  );
  await keyboardScrub();
  assert.match(await page.getByTestId('served').textContent(), /^1/);
  assert.match(await page.getByTestId('coins').textContent(), /12/);
  await page.locator('.db-bath').nth(1).click();
  // Real pointer capture: release outside the scrub control still finishes the bath.
  const scrub = page.locator('.db-scrub');
  await scrub.scrollIntoViewIfNeeded();
  const box = await scrub.boundingBox();
  await page.mouse.move(box.x + box.width / 2, box.y + 20);
  await page.mouse.down();
  await page.clock.runFor(1300);
  await page.mouse.move(box.x - 30, box.y);
  await page.mouse.up();
  assert.match(await page.getByTestId('served').textContent(), /^2/);
  assert.match(await page.getByTestId('coins').textContent(), /22/);
  await page
    .getByRole('button', { name: 'Refill dust & treats', exact: true })
    .click();
  await page.clock.runFor(3100);
  assert.match(await page.locator('.db-supplies').textContent(), /6\/6.*3\/3/);
  console.log(
    'PASS desktop: tutorial, seating, keyboard sweet spot, sneeze neighbor splash, treat repair, pointer capture, coins, refill',
  );
  // Two-tap mode is also a switch-control alternative. Earn all upgrades through UI only.
  await page.getByRole('checkbox').check();
  for (let i = 0; i < 7; i++) {
    await seat();
    await scrub.click();
    await page.clock.runFor(1300);
    await scrub.click();
    if (i === 4) {
      await page
        .getByRole('button', { name: 'Refill dust & treats', exact: true })
        .click();
      await page.clock.runFor(3100);
    }
  }
  assert.match(await page.getByTestId('served').textContent(), /^9/);
  await page.clock.runFor(180000);
  assert.match(await page.getByTestId('clock').textContent(), /No rush/);
  assert.equal(await page.locator('.db-guest').count(), 4);
  await page.getByRole('button', { name: /Close cozy spa/ }).click();
  assert.match(
    await page.locator('.db-welcome').textContent(),
    /9 guests pampered/,
  );
  for (const cost of [30, 40, 25])
    await page
      .getByRole('button', { name: `${cost} coins · Add to spa`, exact: true })
      .click();
  assert.equal(
    await page
      .getByRole('button', { name: '✓ Installed', exact: true })
      .count(),
    3,
  );
  assert.equal(await page.locator('.db-leafy').count(), 1);
  assert.equal(await page.locator('.db-tub i').count(), 3);
  assert.match(await page.getByTestId('coins').textContent(), /11/);
  console.log(
    'PASS cozy: 180 seconds without departures, nine earned services, all three purchases, visible towels/decor and exact coin balance',
  );
  await page.getByRole('button', { name: /Open spa/ }).click();
  assert.match(await page.getByTestId('clock').textContent(), /2:00/);
  assert.match(await page.getByTestId('served').textContent(), /^0/);
  await page.clock.runFor(2100);
  await page.getByRole('button', { name: 'Pause', exact: true }).click();
  assert.equal(
    await page
      .locator('.db-paused p')
      .evaluate((el) => getComputedStyle(el).color),
    'rgb(91, 107, 93)',
  );
  const clock = await page.getByTestId('clock').textContent();
  await page.clock.runFor(5000);
  assert.equal(await page.getByTestId('clock').textContent(), clock);
  await page.getByRole('button', { name: 'Resume', exact: true }).click();
  await seat();
  await scrub.click();
  await page.clock.runFor(500);
  await page.evaluate(() => window.dispatchEvent(new Event('blur')));
  await page.getByRole('button', { name: 'Resume', exact: true }).waitFor();
  assert.equal(
    await page
      .getByRole('meter', { name: 'Scrub pressure', exact: true })
      .getAttribute('aria-valuenow'),
    '0',
  );
  await page.getByRole('button', { name: 'Resume', exact: true }).click();
  await page.getByRole('button', { name: 'How to play', exact: true }).click();
  const guideClock = await page.getByTestId('clock').textContent();
  await page.clock.runFor(2000);
  assert.equal(await page.getByTestId('clock').textContent(), guideClock);
  await page.getByRole('button', { name: 'Close guide', exact: true }).click();
  await page.getByRole('button', { name: 'Resume', exact: true }).click();
  // Trigger the actual visibility handler without a production test hook.
  await page.evaluate(() => {
    Object.defineProperty(document, 'hidden', {
      configurable: true,
      value: true,
    });
    document.dispatchEvent(new Event('visibilitychange'));
    delete document.hidden;
  });
  await page.getByRole('button', { name: 'Resume', exact: true }).waitFor();
  await page.getByRole('button', { name: 'Resume', exact: true }).click();
  await page.clock.runFor(120000);
  assert.equal(await page.getByTestId('clock').textContent(), '0:00');
  assert.match(
    await page.locator('.db-welcome').textContent(),
    /Small spa. Big feelings/,
  );
  await page.getByRole('button', { name: /Open spa/ }).click();
  assert.equal(await page.getByTestId('clock').textContent(), '2:00');
  assert.equal(await page.locator('.db-tub i').count(), 3);
  await page.getByRole('button', { name: 'Pause', exact: true }).click();
  await mkdir(process.env.JCODE_SCRATCH_DIR || '.checks', { recursive: true });
  await page.screenshot({
    path: `${process.env.JCODE_SCRATCH_DIR || '.checks'}/dust-bath-desktop.png`,
    fullPage: true,
  });
  await page.getByRole('link', { name: /MAIN ARCADE/ }).click();
  assert.equal(await page.locator('.arcade-card').count(), 11);
  assert.match(await page.locator('.arcade-hero').textContent(), /Eleven ways/);
  await page.locator('a.arcade-card[href="/dust-bath"]').click();
  assert.equal(await page.getByRole('heading', { level: 1 }).count(), 1);
  console.log(
    'PASS shift: pause/resume, blur cancels hold, guide pause, hidden-tab handler, full timed ending, restart, upgrade carryover, eleventh-card navigation',
  );
  await page.close();
  const mobile = await browser.newPage({
    viewport: { width: 390, height: 844 },
    isMobile: true,
    hasTouch: true,
    reducedMotion: 'reduce',
  });
  mobile.on('pageerror', (e) => errors.push(e.message));
  await mobile.clock.install();
  await mobile.goto(`${base}/dust-bath`);
  await mobile.getByRole('button', { name: /Untimed cozy/ }).tap();
  await mobile.locator('.db-guest').first().tap();
  await mobile.locator('.db-bath').first().tap();
  // Native Chromium touch events exercise the default hold and capture path.
  const touch = await mobile.context().newCDPSession(mobile);
  await mobile.locator('.db-scrub').scrollIntoViewIfNeeded();
  const target = await mobile.locator('.db-scrub').boundingBox();
  await touch.send('Input.dispatchTouchEvent', {
    type: 'touchStart',
    touchPoints: [{ x: target.x + target.width / 2, y: target.y + 20 }],
  });
  await mobile.getByRole('button', { name: /RELEASE IN SWEET SPOT/ }).waitFor();
  await mobile.clock.runFor(1300);
  await touch.send('Input.dispatchTouchEvent', {
    type: 'touchEnd',
    touchPoints: [],
  });
  await mobile.waitForFunction(() =>
    document
      .querySelector('[data-testid="served"]')
      .textContent.startsWith('1'),
  );
  await mobile.locator('.db-guest').first().tap();
  await mobile.locator('.db-bath').first().tap();
  await mobile.locator('.db-scrub').scrollIntoViewIfNeeded();
  const cancelTarget = await mobile.locator('.db-scrub').boundingBox();
  await touch.send('Input.dispatchTouchEvent', {
    type: 'touchStart',
    touchPoints: [
      { x: cancelTarget.x + cancelTarget.width / 2, y: cancelTarget.y + 20 },
    ],
  });
  await mobile.getByRole('button', { name: /RELEASE IN SWEET SPOT/ }).waitFor();
  await mobile.clock.runFor(300);
  await touch.send('Input.dispatchTouchEvent', {
    type: 'touchCancel',
    touchPoints: [],
  });
  await mobile.waitForFunction(
    () =>
      document.querySelector('.db-meter').getAttribute('aria-valuenow') === '0',
  );
  assert.match(await mobile.getByTestId('served').textContent(), /^1/);
  await touch.detach();
  await mobile.getByRole('checkbox').check();
  await mobile.locator('.db-scrub').tap();
  await mobile.clock.runFor(1300);
  await mobile.locator('.db-scrub').tap();
  assert.match(await mobile.getByTestId('served').textContent(), /^2/);
  assert(
    await mobile.evaluate(
      () => document.documentElement.scrollWidth <= innerWidth,
    ),
    '390px viewport must not overflow',
  );
  await mobile.setViewportSize({ width: 320, height: 740 });
  assert(
    await mobile.evaluate(
      () => document.documentElement.scrollWidth <= innerWidth,
    ),
    '320px viewport must not overflow',
  );
  await mobile.setViewportSize({ width: 390, height: 844 });
  await mobile.screenshot({
    path: `${process.env.JCODE_SCRATCH_DIR || '.checks'}/dust-bath-mobile.png`,
    fullPage: true,
  });
  await mobile.close();
  assert.deepEqual(errors, [], 'no browser runtime errors');
  console.log(
    'PASS mobile: native touch hold/release, touch cancellation, two-tap service, 390px and 320px no horizontal overflow, reduced motion, zero page errors',
  );
} finally {
  await browser.close();
}
