...
try {
  ...
  assert.equal(await page.locator('.arcade-card').count(), 13);
  ...
  console.log(
    'PASS Mountain Retreat browser: idle income, unlock, keyboard allocations, expedition/festival pauses and rewards, persistence, offline cap/no double claim, malformed/blocked storage, unrelated save preservation, 13-card navigation, 320/390/768 responsive widths, touch and reduced motion.',
  );
} finally {
  await browser.close();
}
